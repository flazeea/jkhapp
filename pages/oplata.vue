      ...
      <div class="flex justify-center gap-6 mb-6">
        ...
        <div class="flex items-center gap-2">
          <div class="h-9 w-9 rounded-full bg-[#F4F6FF] flex items-center justify-center">
            <Icon name="mingcute:list-check-2-line" class="text-[#C2C7F0] text-xl" />
          </div>
          <div class="h-px w-8 bg-[#E0E4FF]" />
        </div>
        <div>
      ...
      <div class="flex-1 flex flex-col max-h-[350px]">
            <div class="picker-wrapper">
                ...
                <div class="picker-column">
                    ...
                    <div class="picker-scroller" id="deviceScroller"></div>
                </div>
            </div>
        </div>
...
let selectedDevice = ref(devices[0]);
...
class WheelPicker {
        ...
        onMounted(() => {
            const devicePicker = new WheelPicker(
                document.getElementById('deviceScroller'),
                devices,
                0,
                (device, index) => {
                    selectedDevice.value = device;
                    console.log('Выбран прибор:', device.name);
                }
            );
        });

        const handleContinue = () => {
            console.log('Переход на выбор периода с прибором:', selectedDevice.value.name);
            router.push('/period');
        };
</script>
