<template>
  <div class="bg-white w-full min-h-screen flex flex-col">
    ...
    <main class="flex-1 px-4 pt-6 pb-4 flex flex-col pt-30">
      ...
      <div class="flex justify-center gap-1 mb-6">
        <div class="flex items-center gap-2">
          <div class="h-9 w-9 rounded-full border border-[#6B89E1] bg-white flex items-center justify-center">
            <Icon name="mingcute:hashtag-line" class="text-[#6B89E1] text-xl" />
          </div>
          <div class="h-px w-8 bg-[#E0E4FF]" />
        </div>
        <div class="flex items-center gap-2">
          <div class="h-9 w-9 rounded-full bg-[#F4F6FF] flex items-center justify-center">
            <Icon name="mingcute:device-line" class="text-[#C2C7F0] text-xl" />
          </div>
          <div class="h-px w-8 bg-[#E0E4FF]" />
        </div>
        <div class="flex items-center gap-2">
      ...
      <!-- Список услуг (прокручиваемый, как рулетка: выбор по центру) -->
      <div class="flex-1 flex flex-col max-h-[350px]">
            <div class="picker-wrapper">
                ...
                <div class="picker-column">
                    ...
                    <div class="picker-scroller" id="serviceScroller"></div>
                </div>
            </div>
        </div>
...
let selectedService = ref(services[0]);
...
class WheelPicker {
        ...
        onMounted(() => {
            const servicePicker = new WheelPicker(
                document.getElementById('serviceScroller'),
                services,
                0,
                (service, index) => {
        ...
        const handleContinue = () => {
            console.log('Переход на выбор прибора учета с услугой:', selectedService.value.name);
            router.push('/readings-step2');
        };
</script>
